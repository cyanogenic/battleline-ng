...
### 33. 移动端横屏改成上下双层作战台并实时缩放

**动机**

- 现有页面主要围绕桌面三栏设计，放到手机横屏上信息块太多、纵向占用太高，实际观感和操作都很吃力。
- 你明确希望移动端横屏改成：
  - 上半区只看战线
  - 下半区从左到右依次是 `Hand / Orders / Seats`
- 另外，因为手牌还要支持拖动，页面不能再靠纵向滚动“凑空间”，而应该实时缩放到一屏内完成操作。

**改动前预期**

- 将横屏移动端重构为真正的“两层布局”，而不是在桌面三栏基础上硬压缩。
- 顶栏下方不再出现页面纵向滚动。
- 继续保证主战区至少完整显示 3 条战线。
- 出牌、确认、查看座位信息这些关键操作仍然保持清晰可达。
...
- 在移动端横屏下切换成新的两层作战台：
  - 上层：`Battlefield` 横向展开
  - 下层：`Hand / Orders / Seats`
- 战线卡、已出牌小卡、手牌、顶栏和底部控制区都接入了基于可用高度的缩放规则。
- 视口变化、横竖屏切换，以及战局内容变化后，页面会重新计算：
  - 当前可用高度
  - 移动横屏缩放因子
  - 下层控制区高度
- 缩放时不只看整页外框，还会同时检查：
  ...
  - `Orders`
  - `Seats`
  这些区域自己的内容溢出情况，然后按最紧的那个区域继续收缩。
- 为了让移动横屏更紧凑：
  - 隐藏了各面板的长副标题
  - 左右折叠按钮在非桌面断点下不再显示
  - `Seats` 面板在横屏下改成双卡并排，减少纵向压力

**结果与预期对比**

- 与预期一致。
- 这一轮不是简单地“继续压缩桌面 UI”，而是第一次真正把移动端横屏当成独立操作场景来设计。
- 页面现在会优先保证：
  - 无纵向页面滚动
  - 战线主视图优先
  - 下层操作区稳定存在
- 这也为后续继续微调横屏密度、按钮大小和拖拽手感打下了更稳的基础。
